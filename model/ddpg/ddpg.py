...
class DDPG(object):
    ...
    def __init__(self, tasks_num, task_dim, vms_num, vm_dim):
        ...
        self.s_dim = self.task_dim + self.vms_num * self.vm_dim  # 状态维度
        self.s_task_dim = self.task_dim
        self.s_vm_dim = self.vms_num * self.vm_dim
        self.a_dim = self.vms_num  # 动作维度
...
class CNet(nn.Module):  # 通过 s,a 预测出 q
    ...
    def one_hot(self, a):
        a_shape = a.shape
        one_hot_tensor = torch.zeros(a_shape[0], self.adim)
        for i in range(a_shape[0]):
            one_hot_tensor[i][int(a[i][0])] = 1
