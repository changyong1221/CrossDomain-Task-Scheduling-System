...
class DQNScheduler(Scheduler):
    def __init__(self, machine_num, task_batch_num):
        ...
        self.DRL = DQN(self.task_dim, machine_num, self.machine_dim, self.double_dqn, self.dueling_dqn,
                       self.optimized_dqn, self.prioritized_memory)
    ...
    def learn(self, task_instance_batch, machines_id):
        ...
        if self.cur_step > 10:
            ...
            loss = self.DRL.learn()
            print("step:", self.cur_step, ", loss:", loss)
        self.cur_step += 1
