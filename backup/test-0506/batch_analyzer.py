...
def plot_task_batch_results():
    ...
    schedulers = ["DQNScheduler", "DQNScheduler", "DQNScheduler", "DDPGScheduler", "RoundRobinScheduler", "RandomScheduler", "EarliestScheduler", "HeuristicScheduler"]
    labels = ["D3QN-OPT", "D3QN", "DQN", "DDPG", "RR", "Random", "Earliest", "GA"]
    ...
    # GoCJ [300, 350],
    # GoCJ5000batches40concurrency [3000, 3100],
    slice_start = 4100
    slice_end = 4200
    check_and_build_dir(f"./slice{slice_start}-slice{slice_end}")
    for i, scheduler in enumerate(schedulers):
        data_path = f"{labels[i]}/test/task_run_results/{glo.current_dataset}{glo.records_num}/{scheduler}/task_batches/" \
    ...
    dest_path = f"slice{slice_start}-slice{slice_end}/batch_average_task_processing_time_comparison_on_{glo.current_dataset}{glo.records_num}_slice{slice_start}-slice{slice_end}.png"
    plt_config = PltConfig()
    plt_config.title = f"average task processing time comparison on {glo.current_dataset}{glo.records_num}"
    ...
    dest_path = f"slice{slice_start}-slice{slice_end}/batch_makespan_comparison_on_{glo.current_dataset}{glo.records_num}_slice{slice_start}-slice{slice_end}.png"
    plt_config = PltConfig()
    plt_config.title = f"makespan comparison on {glo.current_dataset}{glo.records_num}"
    plt_config.xlabel = "number of task batches"
    ...
    dest_path = f"slice{slice_start}-slice{slice_end}/batch_average_machine_worktime_comparison_on_{glo.current_dataset}{glo.records_num}_slice{slice_start}-slice{slice_end}.png"
    plt_config = PltConfig()
    plt_config.title = f"average machine worktime on {glo.current_dataset}{glo.records_num}"
    plt_config.xlabel = "number of task batches"
...
def plot_throughoutput_comparison():
    schedulers = ["DQNScheduler", "DQNScheduler", "DQNScheduler", "DQNScheduler", "DQNScheduler", "DQNScheduler", "DQNScheduler", "DQNScheduler", "DDPGScheduler", "RoundRobinScheduler", "RandomScheduler", "EarliestScheduler", "HeuristicScheduler", "HeuristicScheduler"]
    labels = ["D3QN-OPT4", "D3QN-OPT3", "D3QN-OPT2", "D3QN-OPT", "D3QN", "DQN", "DQN2", "DQN3", "DDPG", "RR", "Random", "Earliest", "GA", "GA2"]
    ...
    throughoutput_list = []
    for i, scheduler in enumerate(schedulers):
        data_path = f"{labels[i]}/test/machine_status_results/{glo.current_dataset}{glo.records_num}/{scheduler}/"
        machine_num = 20
        max_makespan = 0
    ...
    dest_path = f"throughoutput_comparison_on_GoCJ5000-8.png"
    plt_config = PltConfig()
    ...
    save_to_histogram_from_list(throughoutput_list, dest_path, plt_config, show=False, show_text=True)
...
def plot_average_task_processing_time_comparison():
    ...
    for i, scheduler in enumerate(schedulers):
        data_path = f"{labels[i]}/test/task_run_results/{glo.current_dataset}{glo.records_num}/{scheduler}/" \
    ...
    dest_path = f"total_average_task_processing_time_comparison_on_{glo.current_dataset}{glo.records_num}-8.png"
    plt_config = PltConfig()
...
def plot_batch_results_avg_task_processing_time():
    ...
    for i, scheduler in enumerate(schedulers):
        data_path = f"{labels[i]}/test/task_run_results/{glo.current_dataset}{glo.records_num}/{scheduler}/task_batches/" \
                  f"task_batches_run_results2.txt"
        data = pd.read_csv(data_path, header=None, delimiter='\t')
        data.columns = ['batch_avg_task_processing_time', 'makespan', 'avg_worktime']
        avg_processing_time_data = data['batch_avg_task_processing_time'].tolist()
        data_vector.append(avg_processing_time_data)
    dest_path = f"batch_average_task_processing_time_comparison_on_{glo.current_dataset}{glo.records_num}.png"
    plt_config = PltConfig()
    plt_config.title = f"average task processing time on {glo.current_dataset}{glo.records_num}"
    plt_config.xlabel = "test batch size"
    plt_config.ylabel = "average task processing time"
    plt_config.x_axis_data = [str(i) for i in range(5000)]
