...
def plot_throughoutput_comparison():
    ...
    # 获取总任务数
    data_path = f"D3QN-OPT/test/task_run_results/GoCJ5000/DQNScheduler/" \
    ...
    for i, scheduler in enumerate(schedulers):
        data_path = f"{labels[i]}/test/machine_status_results/GoCJ5000/{scheduler}/"
        machine_num = 20
        max_makespan = 0
    ...
    dest_path = f"throughoutput_comparison_on_GoCJ5000.png"
    plt_config = PltConfig()
    plt_config.title = f"throughoutput comparison on GoCJ5000"
...
def plot_average_task_processing_time_comparison():
    ...
    for i, scheduler in enumerate(schedulers):
        data_path = f"{labels[i]}/test/task_run_results/GoCJ5000/{scheduler}/{scheduler}_task_run_results2.txt"
        data = pd.read_csv(data_path, header=None, delimiter='\t')
    ...
    dest_path = f"total_average_task_processing_time_comparison_on_GoCJ5000.png"
    plt_config = PltConfig()
    plt_config.title = f"total average task processing time comparison on GoCJ5000"
    plt_config.xlabel = "scheduling algorithms"
