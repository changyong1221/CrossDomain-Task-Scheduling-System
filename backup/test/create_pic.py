...
def plot_data_loss():
    data = pd.read_csv("9/loss.txt", header=None)
    data.columns = ['loss']
    ...
    # plt.legend(loc='best')
    # 保存图片
    plt.savefig(f"9/training_loss.png")
    # 展示折线图
    # plt.show()

def plot_data_reward():
    data = pd.read_csv("9/reward.txt", header=None)
    data.columns = ['reward']
    ...
    # plt.legend(loc='best')
    # 保存图片
    plt.savefig(f"9/training_reward_3.png")
    # 展示折线图
    # plt.show()

def plot_data_q_value():
    data = pd.read_csv("9/q_value.txt", header=None)
    ...
    # plt.legend(loc='best')
    # 保存图片
    plt.savefig(f"9/training_q_value.png")
    # 展示折线图
    # plt.show()

def plot_action():
    data = pd.read_csv("9/action.txt", header=None, delimiter='\t')
    data.columns = ['task_mi', 'action']
    data_list = data['action'].tolist()
    # 指定画布大小
    plt.figure(figsize=(10, 4))
    # 设置图标标题
    plt.title(u"Training action")
    # 设置图表x轴名称和y轴名称
    plt.xlabel(u"epoch")
    plt.ylabel("action")
    # 将数据进行处理，以便于画图（需要将DataFrame转为List）
    ...
    # plt.legend(loc='best')
    # 保存图片
    plt.savefig(f"9/training_action.png")
    # 展示折线图
    # plt.show()
